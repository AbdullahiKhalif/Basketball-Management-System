...
function getAllUsers($conn){
    $data = array();
    $message = array();
    $query = "SELECT userId, username, email, password, phone, role, image, status FROM users ";
...
function registerUser($conn) {
    ...
    if ($checkResultUsername->num_rows > 0) {
        ...
        $query = "INSERT INTO users (userId, username, email, password, phone, role, image, status) VALUES ('$newId', '$username', '$email', '$password', '$phone', '$role', '$savedName', '$status')";
...
function updateUser($conn) {
    ...
    if (count($errorMessage) <= 0) {
        $checkUsername = "SELECT * FROM users WHERE username = '$username' AND userId != '$userId'";
        $checkEmail = "SELECT * FROM users WHERE email = '$email' AND userId != '$userId'";
        $checkPhone = "SELECT * FROM users WHERE phone = '$phone' AND userId != '$userId'";
        $checkResultUsername = $conn->query($checkUsername);
        $checkResultEmail = $conn->query($checkEmail);
        $checkResultPhone = $conn->query($checkPhone);
        ...
        if ($checkResultUsername->num_rows > 0) {
            ...
            $query = "UPDATE users SET username = '$username', email = '$email', password = '$password', phone = '$phone', role = '$role', image = '$savedName', status = '$status' WHERE userId = '$userId'";
        ...
        if ($checkResultPhone->num_rows > 0) {
            ...
            if ($checkResultUsername->num_rows > 0) {
                ...
                $query = "UPDATE users sET username = '$username', email = '$email', password = '$password', phone = '$phone', role = '$role', status = '$status' WHERE userId = '$userId'";
...
function deleteUser($conn){
    ...
    if($result) {
        unlink('../uploads/users/'. $userId.".png");
